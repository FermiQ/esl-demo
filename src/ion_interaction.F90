module ion_interaction_esl
  ...
  type ion_interaction_t
   real(kind=dp) :: alpha !< Ewald sum parameter
   contains
  ...
  contains
   ...
   subroutine calculate_periodic(this, sys, forces, eii)
     ...
     real(kind=dp),           intent(out) :: eii

     integer :: iatom, jatom, is, js, ncopy, ic, idim
     integer :: ss, igx, igy, igz, isph
     real(kind=dp) :: dist, zi, zj, rcut, rcopy(3), erfc, charge
     real(kind=dp) :: factor, gx, glen2, gg(3)
     complex(kind=dp) :: sumat, aa, tmp(3), phase(sys%natoms)
   ...
   subroutine calculate_isolated(this, sys, forces, eii)
     ...
     real(kind=dp) :: dist, zi, zj, dd, r(3), f(3)
...
end module ion_interaction_esl
